...
- `isPreEvent()` в app.js — функция существует, но **отключена** в `renderToday()` с 1 июня: проверка убрана, сразу показывается экран «Сегодня»
- `renderCountdown()` — код сохранён, но не вызывается
- Экран «Сегодня» для дня 1 (2 июня) содержит блок паспортного контроля (только RU)
...
- FAB-кнопка помощи: **удалена** по просьбе организатора
...
- `flag` — всегда `''` (флаги стран убраны у всех экспонентов)
...
- [x] Блок рекомендации по паспортному контролю — на экране «Сегодня» день 1 (только RU)
- [x] Гала-ужин 4 июня: время обновлено на 20:00–02:00
- [x] Обратный отсчёт отключён с 1 июня, сразу показывается «Сегодня 2 июня»
- [x] Погода обновлена по прогнозу: 2 июня ☀️+28°C, 3 июня ⛅+28°C, 4 июня ☀️+29°C
