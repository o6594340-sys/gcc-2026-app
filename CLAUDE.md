...
| `exhibitors` | 🤝 Экспоненты | карточки 23 участников с контактами (без фильтров) |
...
- 23 компании, отсортированы в английском алфавитном порядке
...
- `contact` — до 2 имён представителей (из файла Sellers.xlsx, лист MARINA)
- `desc` — 2 предложения по-русски
- Телефоны/мессенджеры пустые — заполнить когда организатор пришлёт
- Возможно добавление ещё 1–2 компаний позже

## Планы (ещё не сделано)
- [ ] Кнопка с ссылкой на Google Form (ждём ссылку от организатора — 2 формы: меню + экскурсии)
